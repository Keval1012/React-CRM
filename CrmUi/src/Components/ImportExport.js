...
import { GET_SAMPLE_DOWNLOAD_FILE } from '../GraphQL/ImportExport/Query';
...
const ImportExport = ({ handleExport, fetchData, screenType }) => {
    ...
    const { data } = useQuery(GET_SAMPLE_DOWNLOAD_FILE, {
        variables: { input: screenType }
    });
    ...
    const handleDownload = async () => {
        ...
        exportExcelResponseForGraphQL(res);
        setImportFileModalOpen(false);
    };
...
// For GraphQL Api
export const exportExcelResponseForGraphQL = (res) => {
    if (res?.sampleData?.status === true) {
        var binaryData = atob(res?.sampleData?.data);
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = res?.sampleData?.fileName;
        a.click();

        window.URL.revokeObjectURL(url);
        message.success(res?.sampleData?.message);
        return;
    }
};