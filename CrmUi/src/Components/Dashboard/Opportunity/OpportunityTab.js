...
const OpportunityTab = () => {
    ...
    const fetchOpportunities = async () => {
        let res;
        if (currentRole === 'HostAdmin') res = await opportunityData?.opportunities;
        if (currentRole === 'Admin') res = await opportunityData?.opportunitiesByTenantAdmin;
        if (currentRole === 'HostUser' || currentRole === 'User') res = await opportunityData?.opportunitiesByUser;
        if (currentRole !== 'HostAdmin' && currentRole !== 'HostUser' && currentRole !== 'Admin' && currentRole !== 'User') res = await opportunityData?.opportunitiesByTenant;
        if (res?.length > 0) setOpportunityList(res);
    };
