...
import { ADD_OR_UPDATE_ACCOUNT, DELETE_ACCOUNT } from '../GraphQL/Account/Mutation';
...
const Account = ({ screenType }) => {
    ...
    const { data, refetch } = useQuery(GET_ACCOUNTS, {
        variables: { input: (currentRole === 'Admin') ? parseData(currUserData)?.tenantId : parseData(currUserData)?.id }
    });
    const [addOrUpdateAccount] = useMutation(ADD_OR_UPDATE_ACCOUNT);
    ...
    const fetchAccounts = async () => {
        let res;
        if (currentRole === 'HostAdmin') {
            res = await data?.accounts;
        } else if (currentRole === 'Admin') {
            res = await data?.accountsByTenantAdmin;
        } else if (currentRole === 'HostUser' || currentRole === 'User') {
            res = await data?.accountsByUser;
        } else if (currentRole !== 'HostAdmin' && currentRole !== 'HostUser' && currentRole !== 'Admin' && currentRole !== 'User') {
            res = await data?.accountsByTenant;
        }
        if (res?.length > 0) {
            setAccountList(res);
    ...
    const showDeleteConfirm = (record) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await deleteAccount({ variables: { input: record?.id }});
                    if (res?.data?.deleteAccount === true) {
        ...
        if (addAccountName) {
            ...
            if (!isEditAccount) {
                try {
                    ...
                    const res = await addOrUpdateAccount({ variables: { input: data }});
                    if (res?.data?.addOrUpdateAccount) {
