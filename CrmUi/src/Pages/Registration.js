...
import { REGISTER_TENANT } from '../GraphQL/Registration/Mutation';
...
const Registration = () => {
    ...
    const handleRegister = async (val) => {
        try {
            const res = await registerTenant({ variables: { input: {
                name: val.registerUsername,
                password: val.registerPassword,
                email: val.registerEmail,
                mobileNumber: val.registerMobile,
                id: 0
            }}});
            if (res?.data?.registerTenant) {
                localStorage.setItem('registerTenantData', JSON.stringify(res?.data?.registerTenant));
                message.success('Your Registration is Successful !!!');
                return navigate('/verification', {
                    state: {
                        tenantId: res?.data?.registerTenant?.id
                    }
                });
            } else {
                message.error(res?.data?.registerTenant);
            }
