...
import { ADD_OR_UPDATE_LEAD, DELETE_LEAD } from '../GraphQL/Lead/Mutation';
...
const Lead = ({ screenType }) => {
    ...
    const [addOrUpdateLead] = useMutation(ADD_OR_UPDATE_LEAD);
    ...
    const fetchLeads = async () => {
        let res;
        if (currentRole === 'HostAdmin') {
            res = await data?.leads;
        } else if (currentRole === 'Admin') {
            res = await data?.leadsByTenantAdmin;
        } else if (currentRole === 'HostUser' || currentRole === 'User') {
            res = await data?.leadsByUser;
        } else if (currentRole !== 'HostAdmin' && currentRole !== 'HostUser' && currentRole !== 'Admin' && currentRole !== 'User') {
            res = await data?.leadsByTenant;
        }
        if (res?.length > 0) {
            setLeadList(res);
        }
    ...
    const showDeleteConfirm = (record) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await deleteLead({ variables: { input: record?.id }});
                    if (res?.data?.deleteLead === true) {
        ...
        if (addLeadCustomerNeed && addLeadContact) {
            ...
            if (!isEditLead) {
                try {
                    ...
                    const res = await addOrUpdateLead({ variables: { input: data }});
                    if (res?.data?.addOrUpdateLead) {
