...
import { ADD_OR_UPDATE_CONTACT, DELETE_CONTACT } from '../GraphQL/Contact/Mutation';
...
const Contact = ({ screenType }) => {
    ...
    const [addOrUpdateContact] = useMutation(ADD_OR_UPDATE_CONTACT);
    ...
    const fetchContacts = async () => {
        let res;
        if (currentRole === 'HostAdmin') {
            res = await data?.contacts;
        } else if (currentRole === 'Admin') {
            res = await data?.contactsByTenantAdmin;
        } else if (currentRole === 'HostUser' || currentRole === 'User') {
            res = await data?.contactsByUser;
        } else if (currentRole !== 'HostAdmin' && currentRole !== 'HostUser' && currentRole !== 'Admin' && currentRole !== 'User') {
            res = await data?.contactsByTenant;
        }
        if (res?.length > 0) {
            setContactList(res);
    ...
    const showDeleteConfirm = (record) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await deleteContact({ variables: { input: record?.id }});
                    if (res?.data?.deleteContact === true) {
        ...
        if (addContactName && addContactEmail) {
            ...
            if (!isEditContact) {
                try {
                    ...
                    const res = await addOrUpdateContact({ variables: { input: data } });
                    if (res?.data?.addOrUpdateContact) {
