...
import { ADD_OR_UPDATE_TENANT } from '../GraphQL/Tenant/Mutation';
import { ADD_OR_UPDATE_USER } from '../GraphQL/User/Mutation';
...
const Profile = () => {
    ...
    const [updateTenant] = useMutation(ADD_OR_UPDATE_TENANT);
    const [updateUser] = useMutation(ADD_OR_UPDATE_USER);
        ...
        if (currentRole !== 'HostAdmin' && currentRole !== 'HostUser' && currentRole !== 'Admin' && currentRole !== 'User') {
            if (userName && userPassword && userEmail && userMobile) {
                ...
                try {
                    ...
                    const res = await updateTenant({ variables: { input: data }});
            ...
            if (userName && userPassword && userEmail && userMobile) {
                ...
                let data = {
                    ...
                    tenantId: (currentRole !== 'HostAdmin' && currentRole !== 'HostUser') ? parseData(currUserData)?.tenantId : null
                };
                ...
                try {
                    ...
                    const res = await updateUser({ variables: { input: data }});
