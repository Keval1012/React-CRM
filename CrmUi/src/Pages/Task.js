...
import { ADD_OR_UPDATE_TASK, DELETE_TASK } from '../GraphQL/Task/Mutation';
...
const Task = ({ screenType }) => {
    ...
    const [addOrUpdateTask] = useMutation(ADD_OR_UPDATE_TASK);
    ...
    const fetchTasks = async () => {
        let res;
        if (currentRole === 'HostAdmin') {
            res = await data?.tasks;
        } else if (currentRole === 'Admin') {
            res = await data?.tasksByTenantAdmin;
        } else if (currentRole === 'HostUser' || currentRole === 'User') {
            res = await data?.tasksByUser;
        } else if (currentRole !== 'HostAdmin' && currentRole !== 'HostUser' && currentRole !== 'Admin' && currentRole !== 'User') {
            res = await data?.tasksByTenant;
        }
        if (res?.length > 0) {
            setTaskList(res);
        }
    ...
    const showDeleteConfirm = (record) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await deleteTask({ variables: { input: record?.id }});
                    if (res?.data?.deleteTask === true) {
        ...
        if (addTaskSubject && addTaskOwner && addTaskStartDate && addTaskDueDate && addTaskCategory && addTaskStatus) {
            ...
            if (!isEditTask) {
                try {
                    ...
                    const res = await addOrUpdateTask({ variables: { input: data }});
                    if (res?.data?.addOrUpdateTask) {
