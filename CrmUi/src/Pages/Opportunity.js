...
import { GET_OPPORTUNITIES } from '../GraphQL/Opportunity/Query';
import { ADD_OR_UPDATE_OPPORTUNITY, DELETE_OPPORTUNITY } from '../GraphQL/Opportunity/Mutation';
...
const Opportunity = ({ screenType }) => {
    ...
    const { data, refetch } = useQuery(GET_OPPORTUNITIES, {
        variables: { input: (currentRole === 'Admin') ? parseData(currUserData)?.tenantId : parseData(currUserData)?.id }
    });
    const [addOrUpdateOpportunity] = useMutation(ADD_OR_UPDATE_OPPORTUNITY);
    ...
    const fetchOpportunities = async () => {
        let res;
        if (currentRole === 'HostAdmin') {
            res = await data?.opportunities;
        } else if (currentRole === 'Admin') {
            res = await data?.opportunitiesByTenantAdmin;
        } else if (currentRole === 'HostUser' || currentRole === 'User') {
            res = await data?.opportunitiesByUser;
        } else if (currentRole !== 'HostAdmin' && currentRole !== 'HostUser' && currentRole !== 'Admin' && currentRole !== 'User') {
            res = await data?.opportunitiesByTenant;
        }
        if (res?.length > 0) {
    ...
    const showDeleteConfirm = (record) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await deleteOpportunity({ variables: { input: record?.id }});
                    if (res?.data?.deleteOpportunity === true) {
    ...
    const handleOpportunityFormValues = async (form) => {
        ...
        if (addOpportunityName && addOpportunityContact && addOpportunityStage) {
            ...
            let data = {
                ...
                closeDate: addOpportunityCloseDate ? dateFormat(addOpportunityCloseDate) : null,
            ...
            if (!isEditOpportunity) {
                try {
                    ...
                    const res = await addOrUpdateOpportunity({ variables: { input: data }});
                    if (res?.data?.addOrUpdateOpportunity) {
