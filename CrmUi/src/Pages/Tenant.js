...
import { GET_TENANTS } from '../GraphQL/Tenant/Query';
import { ADD_OR_UPDATE_TENANT, DELETE_TENANT } from '../GraphQL/Tenant/Mutation';
...
const Tenant = ({ screenType }) => {
    ...
    const { data, refetch } = useQuery(GET_TENANTS);
    const [addOrUpdateTenant] = useMutation(ADD_OR_UPDATE_TENANT);
    ...
    const fetchTenants = async () => {
        const res = await data?.tenants;
        if (res?.length > 0) {
            setTenantList(res);
            dispatch(setTenantData(res));
        }
    };
    ...
    const showDeleteConfirm = (record) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await deleteTenant({ variables: { input: record?.id }});
                    if (res?.data?.deleteTenant === true) {
        ...
        if (userName && userPassword && userEmail && userMobile) {
            ...
            if (!isEditUser) {
                try {
                    ...
                    const res = await addOrUpdateTenant({ variables: { input: data }});
                    if (res?.data?.addOrUpdateTenant) {
